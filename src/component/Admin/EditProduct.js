...
const EditProduct = () => {
    ...
    const[img, name,description,smallPrice,mediumPrice,largePrice, loading, isPress, handelSubmit, onImageChange, onChangeName,onChangeDescription,onChangeSmallPrice,onChangemediumPrice,onChangeLargePrice]=EditProductHook(ProductId)

  return (
    <div dir='rtl' className="AddProduct">
    <div className="container">
        <h2 className='mb-4'>تعديل منتج</h2>
        <div className="product-name  mb-4 d-flex flex-column align-items-start">
                <label className='mb-3' htmlFor="product-pic">صورة المنتج</label>
               <input onChange={(e)=> onImageChange(e)} value={img}  placeholder='صورة المنتج (URL)' type="text" name="product-pic" id="product-pic" />
            </div>
        <div className="product-name  mb-4 d-flex flex-column align-items-start">
